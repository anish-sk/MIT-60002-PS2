...
def get_best_path(digraph, start, end, path, max_dist_outdoors, best_dist, best_path):
    ...
    Returns:
        ...
        a list of building numbers (in Nodes), [n_1, n_2, ..., n_k],int,int
    ...
    if start==end:
        ...
        for edge in digraph.get_edges_for_node(start):
            if edge.get_destination() not in path[0]:
                if (path[1] + int(edge.get_total_distance()) <= best_dist) and (path[2] + int(edge.get_outdoor_distance() )<= max_dist_outdoors):
                    new_dist= path[1] + int(edge.get_total_distance()) 
                    new_outdoor= path[2] + int(edge.get_outdoor_distance())
                    new_path=get_best_path(digraph, edge.get_destination(), end, [path[0],new_dist,new_outdoor], max_dist_outdoors, best_dist, best_path)
                    if new_path!=None:    
...
def directed_dfs(digraph, start, end, max_total_dist, max_dist_outdoors):
    ...
    bestpath=get_best_path(digraph, start, end, [[],0,0], max_dist_outdoors, max_total_dist, None)
    if bestpath != None and bestpath[1] <= max_total_dist:
        return [node.get_name() for node in bestpath[0]]
    else:
